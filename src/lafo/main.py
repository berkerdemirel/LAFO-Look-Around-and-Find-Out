...
from lafo.utils.score_functions import LAFO  # , fDBD, knn_score
...
def cache_run(cfg, device):
    ...
    feat_log = torch.from_numpy(
        np.memmap(f"{cache_dir}/feat.mmap", dtype=float, mode="r", shape=(id_train_size, 512))
    ).to(device)
    score_log = torch.from_numpy(
        np.memmap(f"{cache_dir}/score.mmap", dtype=float, mode="r", shape=(id_train_size, num_classes))
    ).to(device)
    ...
    score_log_val = torch.from_numpy(
        np.memmap(f"{cache_dir}/score.mmap", dtype=float, mode="r", shape=(id_val_size, num_classes))
    ).to(device)
    ...
    ood_feat_score_log = {}
    ood_dataset_size = {"SVHN": 26032, "iSUN": 8925, "places365": 10000, "dtd": 5640}
    ...
    for ood_dataset in cfg.out_datasets:
        ...
        ood_score_log = torch.from_numpy(
            np.memmap(
                ...
                dtype=float,
                mode="r",
                shape=(ood_dataset_size[ood_dataset], num_classes),
            )
        ).to(device)
